...
| 폴더 | 첫 번째 (`a.png` 가 됨) | 두 번째 (`b.png` 가 됨) |
|---|---|---|
| `ok/` | `nvidia_970.bin` | `nvidia_970_again.bin` |
| `real/` | `nvidia_970.bin` | `amd_mi300x.bin` |
| `broken/` | `nvidia_970.bin` | `broken_demo.bin` |

**첫 번째 자리는 항상 `nvidia_970.bin` 이다.** 기준을 하나로 고정해야
세 그림을 나란히 비교할 수 있다.
...
### 어느 `.bin` 이 어느 PNG 가 되는가

명령에 넣는 **순서**로 정해진다.

```bash
python3 scripts/40_demo_images.py  nvidia_970.bin  amd_mi300x.bin
                                   ─────┬──────    ─────┬──────
                                     첫 번째          두 번째
                                      = a.png         = b.png
```

그리고 `compare.png` 에서의 자리도 그 순서 그대로다:
...
| 파일 | 내용 | 크기 |
|---|---|---|
| `a.png` | **첫 번째**로 넣은 `.bin` 의 열지도 | 384 × 384 px |
| `b.png` | **두 번째**로 넣은 `.bin` 의 열지도 | 384 × 384 px |
| `diff.png` | **두 값의 차이** \|a − b\| | 384 × 384 px |
| `compare.png` | 위 셋을 가로로 붙인 것 | **1184 × 384 px** |

### "`.bin` 을 그림으로 그린다"가 무슨 뜻인가

`.bin` 안에는 숫자 65,536개가 이렇게 들어 있다:

```
0.0231  -0.0087  0.1442  0.0009  -0.0553  ...
```

그대로 보여주면 아무도 못 읽는다. 그래서 **숫자 하나를 픽셀 하나로** 바꾼다 —
값이 작으면 어둡게, 크면 밝게. 64행 × 64열을 격자로 늘어놓으면 열지도 한 장이 된다.

**숫자 6만 개를 대조하는 건 사람이 못 하지만, 그림 두 장이 같은지는 눈으로 1초 만에 안다.**
이 도구의 존재 이유가 그것이다.

**발표에는 `compare.png` 한 장만 쓴다.** 나머지 셋은 그 재료다.

### ⚠ `compare.png` 는 왜 따로 있는가 — 데이터는 중복인데

`a`·`b`·`diff` 세 장이면 정보는 충분하다. `compare.png` 는 **섞이는 사고를 막는 안전장치**다.

**폴더를 넘나들며 조합하면 안 된다.** 예를 들어 `real/a.png` 와 `broken/b.png` 를
나란히 놓으면 **비교가 성립하지 않는다.**

이유는 `a.png` 의 색 기준이 **짝꿍과 함께** 정해지기 때문이다:

```python
lo, hi = min(min(a), min(b)), max(max(a), max(b))   # a 와 b 를 합쳐서 범위 결정
```

그래서 **똑같은 `nvidia_970.bin` 에서 나왔는데도** 이렇게 된다:

```
ok/a.png    ==  real/a.png     같다  (짝꿍의 값 범위가 비슷해서)
real/a.png  !=  broken/a.png   ★ 다르다 (buggy 쪽 값 범위가 달라 색 기준이 바뀜)
```

색 잣대가 다른 두 장을 나란히 놓으면 **없는 차이가 있어 보이거나 있는 차이가 가려진다.**
발표 자리에서는 조작으로 오해받을 수도 있다.

`compare.png` 는 **한 번의 실행에서 통째로 생성되므로 섞일 수가 없다.**
부수적으로 세 칸의 높이·간격이 자동으로 맞아 슬라이드에 끌어다 놓기만 하면 된다.

→ **원칙: 발표 자료에는 `compare.png` 만 쓴다.
   `a`·`b`·`diff` 를 직접 조합하지 않는다.**
   (한 칸만 크게 띄우고 싶을 때만 같은 폴더 안에서 꺼내 쓴다)
